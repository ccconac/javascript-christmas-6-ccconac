...
describe('혜택 클래스 테스트', () => {
  ...
  describe('weekendDiscount() 메소드 테스트', () => {
    test('총주문 금액 10,000원부터 평일 할인이 적용되는지 확인한다.', () => {
      ...
      const discount = benefit.weekendDiscount(3, 142000);
      expect(discount).toEqual(-4046);
    });

    test('총주문 금액이 10,000 미만일 경우 평일 할인이 적용되지 않는지 확인한다.', () => {
      order.getOrderedMenus('시저샐러드-1');
      const discount = benefit.weekendDiscount(3, 8000);
      expect(discount).toEqual(false);
    });
    ...
    test.each(WEEKEND_EVENT)(
      ...
      day => {
        order.getOrderedMenus('초코케이크-1');
        const discount = benefit.weekendDiscount(day, 15000);
        expect(discount).toBe(-2023);
      },
    );

    test('디저트 메뉴가 아닌 다른 메뉴를 주문했을 때 할인이 적용되지 않는지 확인한다.', () => {
      order.getOrderedMenus('바비큐립-1');
      const discount = benefit.weekendDiscount(3, 54000);
  ...
  describe('weekdayDiscount() 메소드 테스트', () => {
    test('총주문 금액 10,000원부터 주말 할인이 적용되는지 확인한다.', () => {
      ...
      const discount = benefit.weekdayDiscount(1, 142000);
      expect(discount).toEqual(-4046);
    });

    test('총주문 금액이 10,000 미만일 경우 주말 할인이 적용되지 않는지 확인한다.', () => {
      order.getOrderedMenus('시저샐러드-1');
      const discount = benefit.weekdayDiscount(1, 8000);
      expect(discount).toEqual(false);
    });
    ...
    test.each(WEEKDAY_EVENT)(
      ...
      day => {
        order.getOrderedMenus('바비큐립-1');
        const discount = benefit.weekdayDiscount(day, 54000);
        expect(discount).toBe(-2023);
      },
    );
    ...
    test('메인 메뉴가 아닌 다른 메뉴를 주문했을 때 할인이 적용되지 않는지 확인한다.', () => {
      order.getOrderedMenus('초코케이크-1');
      const discount = benefit.weekdayDiscount(1, 15000);
      expect(discount).toBe(0);
    });
  });
  ...
  describe('circulateBenefit() 메소드 테스트', () => {
    test('총혜택 금액을 올바르게 계산하는지 확인한다.', () => {
      ...
      const result = benefit.circulateBenefit(3, 142000, '샴페인 1개');

      expect(result).toEqual(-31246);
    });

    test('혜택이 적용되지 않은 경우 올바르게 계산하는지 확인한다.', () => {
      order.getOrderedMenus('타파스-1');
      const result = benefit.circulateBenefit(26, 5500, '없음');
  ...
  describe('circulateAfterTotal() 메소드 테스트', () => {
    test('할인 후 예상 결제 금액을 올바르게 계산하는지 확인한다.', () => {
      ...
      const result = benefit.circulateAfterTotal(3, 142000, '샴페인 1개');
    ...
    test('할인 후 예상 결제 금액을 올바르게 계산하는지 확인한다.', () => {
      order.getOrderedMenus('타파스-1');
      const result = benefit.circulateAfterTotal(26, 5500);
