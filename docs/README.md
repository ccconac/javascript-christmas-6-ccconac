...
- [ ] 입력이 메뉴 형식이 예시와 다르면 예외가 발생한다. (e.g. 해산물파스타-2,레드와인-1,초코케이크-1)
- [ ] 입력이 메뉴판에 없는 메뉴이면 예외가 발생한다.
- [ ] 메뉴의 개수가 1 미만의 숫자이면 예외가 발생한다.
- [ ] 메뉴의 개수가 숫자가 아니면 예외가 발생한다.
- [ ] 중복 메뉴를 입력하면(e.g. 시저샐러드-1,시저샐러드-1) 예외가 발생한다.
- [ ] 주문한 메뉴가 20개가 넘어가면 예외가 발생한다.
- [ ] 음료만 주문하면 예외가 발생한다.
